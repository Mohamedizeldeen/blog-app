...
<body class="bg-gray-100 min-h-screen">
    <div class="container mx-auto py-8 px-4 max-w-2xl">
        <div class="bg-white rounded-lg shadow-md p-6">
            <h1 class="text-3xl font-bold text-gray-800 mb-6">Create Interview</h1>
            
            <form action="{{ route('interviews.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('content') border-red-500 @enderror" 
                              id="content" name="content" rows="5" placeholder="Enter interview content..." required>{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
